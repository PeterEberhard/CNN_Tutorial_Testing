...
public class MaxPoolLayer extends Layer{
    ...
    public MaxPoolLayer(int stepSize, int windowSize, int inputLength, int inputRows, int inputColumns) {
    ...
    @Override
    public void backPropagation(double[] dLdO) {
        List<double[][]> matrix = vectorConvertToMatrix(dLdO, getOutputLength(), getOutputRows(), getOutputCols());
    ...
    public void backPropagation(List<double[][]> dLdO) {
        ...
        for(double[][] array: dLdO){
            ...
            for(int r = 0; r < getOutputRows(); r++){
                for(int c = 0; c < getOutputCols(); c++){
                    int max_x = lastMaxRow.get(l)[r][c];
                    int max_y = lastMaxColumn.get(l)[r][c];
    ...
    @Override
    public int getOutputElements() {
        return (inputLength*getOutputCols()*getOutputRows());
    }
}